...
The output of the analysis_cdt.C macro is the same root file "cdt.root" that now contains the original event tree plus 2 new event trees. Although ROOT allows to append new branches to an existing tree, for debugging purposes I prefer to leave the tree with the raw data intact and create new trees, one for the "calibrated" data called "cdt_new" and one for the voxel mapping called "cdt_new_cal". The voxel mapping consists of attaching the coordinates x,y,z of the centre of the detector voxel to each neutron event detected by EndCap. These coordinates are read out from the 'endcap_lookup.root' files obtained in the GEANT4 simulation of the EndCap detector.  
