  ...
  */
  ...
  for (Long64_t i = 0; i <= nevents; i++) {
    ...
    if (vchopperTime[i] != 0 && vtime[i] != 0) {
      ...
      // from FP's file (Dtt1 in normal mode) 			tdiff =
      // ntime-nchopperTime + 145230; // tcorr from FP's file (Dtt1 in normal
      // mode)
      tdiff =
          ntime - nchopperTime +
  ...
  // in the root tree) 			sumo 			wire 			seg 			counter
  // strip 			module 			indexi 			indexj
  // posx 			posy 			posz
